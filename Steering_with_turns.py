...
"""
def find_edges_and_curves(img, y, threshold=THRESHOLD, consecutive_pixels=CONSECUTIVE_PIXELS):
    consecutive_count = 0
    left_edge = None
    right_edge = None
    global left_turn, right_turn
    for x in range(0, 20):
        for y in range(100, 240, 10):
            if img.get_pixel(x, y) < THRESHOLD:
                consecutive_count += 1
                if consecutive_count >= CONSECUTIVE_PIXELS:
                    left_turn = True
                    print("Da ist eine Linkskurve")
                    break
            else:
                consecutive_count = 0
                print("Da ist eine Linkskurve!")
    for x in range(0, 20):
        for y in range(100, 240, 10):
            if img.get_pixel(x, y) < THRESHOLD:
                ...
                if consecutive_count >= CONSECUTIVE_PIXELS:
                    right_turn = True
                    print("Da ist eine Rechtskurve!")
                    break
            else:
                consecutive_count = 0
    if left_turn != True and right_turn != True:
        width = img.width()
        mid_x = width // 2
...
def find_border(img):
    # Define the y-zones
    for y in range(2, 22, 10):  # The pixels/lines at the very top and bottom are ignored
        edges = []

        left, right = find_edges_and_curves(img, y)
