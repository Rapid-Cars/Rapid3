...
THRESHOLD = 50  # Brightness threshold to detect dark pixels (0-255)
...
def find_edges_and_turns(img, y, threshold=THRESHOLD, consecutive_pixels=CONSECUTIVE_PIXELS):
    #img.binary([(0, THRESHOLD)])  # Convert to binary image
    consecutive_count = 0
    left_edge = None
    right_edge = None
    ...
    for x in range(310, 320):
        for y_t in range(200, 240):
            if img.get_pixel(x, y_t) == 255:  # White pixel:
                consecutive_count += 1
    ...
    if left_turn == False and right_turn == False:
        ...
        for x in range(mid_x, 0, -1):
                if img.get_pixel(x, y) == 0:  # Black pixel:
                    consecutive_count += 1
